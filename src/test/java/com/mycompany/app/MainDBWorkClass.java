...
public class MainDBWorkClass {
    static final String DB_CONNECTION = "jdbc:mysql://localhost:3306/mySQLHomeWork?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC";
    static final String DB_USER = "root";
    static final String DB_PASSWORD = "iyefrdim12";
    ...
    private static void initDB() throws SQLException {
        ...
        try {
            ...
            st.execute("CREATE TABLE Clients (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, name VARCHAR(20) NOT NULL, age INT, salary FLOAT)");
        } finally {
            st.close();
        }
    }
    ...
    private static void changeClient(Scanner sc) throws SQLException {
        ...
        PreparedStatement ps = conn.prepareStatement("UPDATE Clients SET age = ?, salary = ? WHERE name = ?");
        try {
            ps.setInt(1, age);
            ...
            try {
                ...
                try {
                    for (int i = 0; i < count; i++) {
                        ...
                        ps.setDouble(3, (double) Math.round(rnd.nextDouble()*10000)/100);
    ...
    private static void showSalary() throws SQLException {
        PreparedStatement ps = conn.prepareStatement("SELECT name, salary FROM Clients");
        try {
            // table of data representing a database result set,
            ResultSet rs = ps.executeQuery();
            try {
